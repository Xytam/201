def findMin(inputList,lowestValue):
    if (len(inputList)==1):
        return lowestValue
    newList=list(inputList)
    newList.remove(lowestValue)
    minValue=findMin(newList,newList[0])
    if (minValue<lowestValue):
        lowestValue=minValue
    return lowestValue
def sort(shuffledList,newList):
    if(len(shuffledList)==0):
        return newList
    minimumValue=findMin(shuffledList,shuffledList[0])
    newList.append(minimumValue)
    shuffledList.remove(minimumValue)
    sort(shuffledList,newList)
    return newList
...
def main():
    ...
    min1 = findMin(jumbledList, jumbledList[0])
    print("The first min is %d" % min1)
    jumbledList.remove(min1)
    min2 = findMin(jumbledList, jumbledList[0])
    print("The second min is %d" % min2)
